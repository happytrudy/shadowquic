...
fn parse_kilo_suffix() {
    assert_eq!(parse_bps("1K").unwrap(), 1_000);
    assert_eq!(parse_bps("1k").unwrap(), 1_000);
    assert_eq!(parse_bps("1.5K").unwrap(), 1_500);
    assert_eq!(parse_bps("30K").unwrap(), 30_000);
}

#[test]
fn parse_mega_suffix() {
    assert_eq!(parse_bps("1M").unwrap(), 1_000_000);
    assert_eq!(parse_bps("1m").unwrap(), 1_000_000);
    assert_eq!(parse_bps("1.5M").unwrap(), 1_500_000);
    assert_eq!(parse_bps("20M").unwrap(), 20_000_000);
}

#[test]
fn parse_giga_suffix() {
    assert_eq!(parse_bps("1G").unwrap(), 1_000_000_000);
    assert_eq!(parse_bps("0.5G").unwrap(), 500_000_000);
    assert_eq!(parse_bps("2G").unwrap(), 2_000_000_000);
}

#[test]
fn parse_rounding() {
    assert_eq!(parse_bps("1.25K").unwrap(), 1_250);
    assert_eq!(parse_bps("1.1K").unwrap(), 1_100);
    assert_eq!(parse_bps("1.9M").unwrap(), 1_900_000);
}
