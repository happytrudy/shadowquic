...
async fn test_http_head_forward() {
    ...
    spawn_mixed_proxy_chain(11080, 11094).await;
...
async fn test_http_get_forward() {
    ...
    let target_addr = spawn_http_target().await;
    spawn_mixed_proxy_chain(11081, 11095).await;

    tokio::time::sleep(Duration::from_millis(100)).await;

    let mut stream = TcpStream::connect("127.0.0.1:11081").await.unwrap();
...
async fn test_http_connect_tunnel() {
    ...
    spawn_mixed_proxy_chain(11082, 11096).await;

    tokio::time::sleep(Duration::from_millis(100)).await;

    let mut stream = TcpStream::connect("127.0.0.1:11082").await.unwrap();
